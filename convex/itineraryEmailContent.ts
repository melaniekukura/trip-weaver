...
export function itineraryEmailContent(snapshot: ItineraryEmailSnapshot) {
  ...
  const itemText = snapshot.items.length ? snapshot.items.map(item => [
    ...
    item.notes,
    safeUrl(item.url),
  ].filter(Boolean).join("\n")).join("\n\n") : "No transportation or activities have been scheduled yet.";
  ...
  const items = snapshot.items.length ? snapshot.items.map(item => {
    ...
    return `<li style="margin:0 0 16px;padding:16px;border-left:4px solid ${item.kind === "transportation" ? "#16CBC4" : "#251F47"};background:#F7F7F7">` +
  ...
  }).join("") : "<p>No transportation or activities have been scheduled yet.</p>";
