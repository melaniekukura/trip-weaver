...
export function LocationPicker({ label, value = "", required = false, disabled = false, clearOnSelect = false, citiesOnly = false, airportsOnly = false, onSelect, onClear }: LocationPickerProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void searchLocations(search, controller.signal, citiesOnly).then((options) => {
  ...
  function choose(option: LocationOption) {
    const selected = citiesOnly ? `${option.city}, ${option.country}` : locationValue(option);
    onSelect(selected);
    setQuery(clearOnSelect ? "" : selected);
    setOpen(false);
    setActive(0);
    input.current?.focus();
  }
    ...
    <div className="location-picker" onBlur={(event) => {
      ...
      {showOptions && <ul className="location-options" id={`${id}-options`} role="listbox" aria-label={`${label} suggestions`}>
        ...
        {!loading && lookup.term === search && !lookup.error && search.trim().length >= 2 && options.length === 0 &&
          <li role="presentation">{citiesOnly ? "No matching cities. Try another city name." : "No matching locations. Try another name or an airport code."}</li>}
        {options.map((option, index) => (
          <li id={`${id}-option-${index}`} key={option.id} role="option" aria-selected={index === active}
            ...
            <span><strong>{option.airport ?? option.city}</strong>
              <small>{citiesOnly ? option.country : option.code ? `${option.city}, ${option.country}` : `${option.country} · All airports`}</small></span>
