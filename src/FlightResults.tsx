...
export function FlightResults({ research, flight, filters, selectedSourceId, onSelectOutbound, onSelectFlight, compact = false }: {
  compact?: boolean;
  ...
  onSelectFlight?: (source: Doc<"researchSources">) => void;
}) {
  ...
  return <div aria-live="polite">
    {!compact && <div className="transportation-results-heading">
    ...
    {research?.run.status === "failed" && <FlightSearchError run={research.run} />}
    {research?.run.status === "completed" && <>
      ...
      <ol className={`transportation-flight-list${compact ? " compact-flight-list" : ""}`}>
        {sources.map((source, index) => <li className={`transportation-flight-card${selectedSourceId === source._id ? " is-selected" : ""}`} key={source._id}>
          <div className="transportation-flight-details">
            {!compact && <span className="flight-rank">{index === 0 ? "Lowest returned fare" : `Option ${index + 1}`}</span>}
            <h4>{source.flight!.airline}</h4>
            {!compact && source.flight!.originAirport && source.flight!.destinationAirport &&
            ...
            <p>{compact ? "" : `${source.flight!.duration} · `}{source.flight!.stops}</p>
          </div>
          <div className="transportation-flight-price">
            ...
            {flight.tripType !== "round-trip" && onSelectFlight && <button type="button" className="secondary-button"
              aria-pressed={selectedSourceId === source._id} onClick={() => onSelectFlight(source)}>
              {selectedSourceId === source._id ? "Selected" : "Select flight"}</button>}
          </div>
        </li>)}
      </ol>
    </>}
