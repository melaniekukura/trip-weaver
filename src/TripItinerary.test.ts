...
test("itinerary groups current booked flights and planned activities by date and time", () => {
  ...
  const lodgings = [{ _id: "lodging" as Id<"lodgings">, destination: "LAX", name: "Ocean Hotel", checkInDate: "2026-10-15",
    checkOutDate: "2026-10-22", booked: true, currency: "USD", tripId: "trip" as Id<"trips">, updatedAt: 1 }] as Doc<"lodgings">[];
  const days = itineraryDays(route, legs, favorites, lodgings);
  expect(days.map(day => day.date)).toEqual(["2026-10-15", "2026-10-22"]);
  expect(days[0].items.map(item => item.title)).toEqual(["DTW to LAX", "Getty Center", "Ocean Hotel"]);
...
test("itinerary page renders the combined schedule and edit paths", () => {
  const favorites = [{ _id: "favorite", item: { kind: "activities", title: "Getty Center", description: "Art", url: "https://example.com", destination: "Los Angeles", retrievedAt: "2026-01-01" }, itinerary: { date: "2026-10-15", time: "13:00" } }];
  const lodgings = [{ _id: "lodging", destination: "LAX", name: "Ocean Hotel", checkInDate: "2026-10-15", checkOutDate: "2026-10-22", booked: true, currency: "USD", tripId: "trip", updatedAt: 1 }];
  query.mockImplementation(reference => getFunctionName(reference) === "lodgings:list" ? lodgings : favorites);
  ...
  const html = renderToStaticMarkup(createElement(TripItinerary, { tripId: "trip" as Id<"trips">, route, plan, onOpenTransportation: vi.fn(), onOpenLodging: vi.fn(), onOpenInterests: vi.fn() }));
  for (const text of ["Your itinerary", "Thursday", "October 15, 2026", "Booked transportation", "DTW to LAX", "ABC123", "Getty Center", "Booked stay", "Ocean Hotel", "Edit transportation", "Edit lodging", "Edit activities"]) expect(html).toContain(text);
});
